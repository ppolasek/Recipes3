...
//MongoError: E11000 duplicate key error index: recipes3.recipe_tag.$tagName_1 dup key: { : "Sandwich" }
...
//       msg: 'E11000 duplicate key error index: recipes3.recipe_tag.$tagName_1 dup key: { : "Sandwich" }'
//   },
//   'return_obj' : [
...
exports.send_response = function (req, res) {
    if ('recipes3' in req && req.recipes3 !== null) {
        if ('output' in req.recipes3) {
            _ok(res, req.recipes3.output);
        } else if ('err' in req.recipes3) {
            _error(res, req.recipes3.err);
        } else {
            _ok(res, null);
        }
    } else {
