...
/**
 * Add the recipes3 object to the request so downstream apps may use it
 * @param req
 * @param res
 * @param next
 */
exports.add_recipes3_obj = function (req, res, next) {
    // logger.debug('req_res_converter.add_recipes3_obj()');
    req.recipes3 = {};
    next();
}
...
exports.response_property_converter = function (req, res, next) {
    // logger.debug('req_res_converter.request_property_converter() req.recipes3: ' + ('recipes3' in req && req.recipes3 !== null ? 'exists and is not null' : 'is null') );
    if ('recipes3' in req && req.recipes3 != null) {
        convert_response_properties(req.recipes3);
    }
    next();
}
